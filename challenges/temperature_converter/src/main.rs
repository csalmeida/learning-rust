...
fn main() {
    ...
    while has_supported_value == false {
      ...
      // Requests user input:
      io::stdin().read_line(&mut convert_to_temperature)
      .expect("Failed to read line");

      // Removes any trailing new lines in input:
      convert_to_temperature = convert_to_temperature.trim().to_string();

      if convert_to_temperature.to_string() == "celsius" || convert_to_temperature.to_string() == "c" {
        ...
        println!("{} Fahrenheit is {} Celsius.", temperature, fahrenheit_to_celsius(temperature));
        ...
        // Need to convert the input back into a number:
        let temperature = temperature.trim().parse().unwrap();
        println!("{} Celsius is {} Fahrenheit", temperature, celsius_to_fahrenheit(temperature));
        has_supported_value = true;
      } else {
